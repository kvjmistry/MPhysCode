...
def xmlread(filename):
    ...
    for j in range(0,Rows):   
        ...
        if (j == 2424 or j == 2425 or j == 7875 or j==78 or j==896): 
            Baseline.append(0); 
            continue;
        ...
        h = 0
        for h in range (60,3940,10): 
            #steps of 10 for LN2 20 for dry
            if ((VoltsNew[h] - VoltsNew[h-12])>0.9) and (VoltsNew[h+16]>0.6)and (VoltsNew[h+20]>0.4  ): 
                #h=10 & >2 for SIPMthree
                u = sum(VoltsNew[h-60:h-30])/30
                
                'find the max peak of the array via mean method' 
                #PeakMax.append(np.max(VoltsNew[p3:p4]) - u )
                if  (max(VoltsNew[h:h+20])- u < 3.8):
                    PeakMax.append(np.max(VoltsNew[h:h+20])- u )

                #DarkCount.append(VoltsNew[h-9:h+40])
           
            
        '''for h in range (60,3940):   
            if ((VoltsNew[h] - VoltsNew[h-12])>10) and (VoltsNew[h+50]>5):
                #h=10 & >2 for SIPMthree
                u = sum(VoltsNew[h-60:h-30])/30
